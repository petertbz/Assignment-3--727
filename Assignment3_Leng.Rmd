...
table_list <- html_nodes(url, xpath = '//table')
#table_list <- html_nodes(url, xpath = '//*[@id="mw-content-text"]/div[1]/table[2]')
#table <- html_table(table_list)
str(table_list)
...
```{r}
places_east <- gsub(pattern = " ", replacement = "_" , x = places_east)
places_east
```
...
for(i in 1:length(places_east)) {
  url <- paste0("https://en.wikipedia.org/wiki/" , places_east[i])
 }
url
```
...
First, scrape just the text without any of the information in the margins or headers. For example, for "Grand Boulevard", the text should start with, "**Grand Bouleavard** on the [South Side](https://en.wikipedia.org/wiki/South_Side,_Chicago "South Side, Chicago") of [Chicago](https://en.wikipedia.org/wiki/Chicago "Chicago"), [Illinois](https://en.wikipedia.org/wiki/Illinois "Illinois"), is one of the ...". Make sure all of the text is in one block by using something like the code below (I called my object `description`).

```{r}
text <- html_nodes(url, xpath = '//p')
#text <- text[[2]]
text <- html_text(text) 
